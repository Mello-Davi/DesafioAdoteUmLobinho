    ...
    div.innerHTML = `
            ...
            <div class="apresentacao">
                <div class="adocao">
                    ...
                    <button class="ver-lobo" ${
                      lobo.adotado ? "disabled" : ""
                    }>${lobo.adotado ? "Adotado" : "Adotar"}</button>
                    </a>
...
function excluirLobo() {
  ...
  botaoExcluir.addEventListener("click", () => {
    ...
    const lobinhoSelecionado = JSON.parse(
      localStorage.getItem("lobinhoSelecionado")
    );
    lobos = JSON.parse(localStorage.getItem("lobos")) || [];

    const index = lobos.findIndex(
      (lobo) => parseInt(lobo.id) === parseInt(lobinhoSelecionado.id)
    );

    if (index !== -1) {
      if (
        confirm(
          `Tem certeza que deseja excluir o lobinho "${lobos[index].nome}"?`
        )
      ) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (path.endsWith("index.html") || path.endsWith("/")) {
    ...
    document.getElementById(
      "idadeLoboA"
    ).textContent = `Idade: ${loboA.idade} anos`;
    ...
    document.getElementById(
      "idadeLoboB"
    ).textContent = `Idade: ${loboB.idade} anos`;
  ...
  if (path.endsWith("listaDeLobinhos.html")) {
    ...
    document
      ...
      ?.addEventListener("click", filtrarLobos);
    document
      .getElementById("meuCheckbox")
      ?.addEventListener("change", filtrarLobos);
  }
  // ========== adotarLobinho.html ==========
    ...
    form.addEventListener("submit", function (e) {
      ...
      const index = lobosAtuais.findIndex(
        (l) => parseInt(l.id) === parseInt(lobinho.id)
      );
  ...
  if (formNovoLobo) {
    formNovoLobo.addEventListener("submit", function (e) {
      ...
      const lobosAtuais = JSON.parse(localStorage.getItem("lobos")) || [];
      const maiorId = lobosAtuais.reduce(
        (max, lobo) => Math.max(max, lobo.id || 0),
        0
      );
